...
export async function GET() {
  try {
    const res = await query("SELECT key, value FROM public.settings WHERE key IN ('hide_team_images', 'hero_bg_type', 'hero_bg_url', 'hero_bg_url_mobile', 'hero_poster_url', 'hero_headline_text', 'hero_headline_visible', 'hero_text_mobile_visible')");
...
export async function POST(req: Request) {
  try {
    ...
    if (body.hero_bg_url_mobile !== undefined) {
      updates.push(['hero_bg_url_mobile', body.hero_bg_url_mobile]);
    }
    ...
    if (body.hero_text_mobile_visible !== undefined) {
      updates.push(['hero_text_mobile_visible', body.hero_text_mobile_visible ? 'true' : 'false']);
    }
